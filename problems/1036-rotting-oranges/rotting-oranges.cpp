...
public:
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        while (!q.empty()) {
            int size = q.size();
            bool rottenOccurred = false;  // Flag to check if any orange rotted in this minute.
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (auto &d : directions) {
                    ...
                    if (newR >= 0 && newR < m && newC >= 0 && newC < n && grid[newR][newC] == 1) {
                        ...
                        q.push({newR, newC});   // Add it to the queue for the next minute.
                        fresh--;                // Decrement the count of fresh oranges.
                        rottenOccurred = true;  // Mark that rotting occurred this minute.
                    }
                }
            }
